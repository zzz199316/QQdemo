...
public class PayPsdInputView extends EditText {
        ...
        protected void onSizeChanged(int w, int h, int oldw, int oldh) {
            ...
            Paint.FontMetrics fontMetrics = circlePaint.getFontMetrics();
            float top = fontMetrics.top;//为基线到字体上边框的距离,即上图中的top
            float bottom = fontMetrics.bottom;//为基线到字体下边框的距离,即上图中的bottom

            startY = (int) (rectF.centerY() - top/2 - bottom/2);//基线中间点的y轴计算公式
        }
        ...
        private void drawPsdCircle(Canvas canvas) {
            ...
            for (int i = 0; i < textLength; i++) {
              //  canvas.drawCircle(startX + i * 2 * startX,startY,radius,circlePaint);
                ...
                canvas.drawText(String.valueOf(texts[i]),startX + i * 2 * startX,startY,circlePaint);
            }
        }
